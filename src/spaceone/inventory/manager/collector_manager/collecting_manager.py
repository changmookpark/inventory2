...
from spaceone.inventory.manager.collector_manager.region_config_manager import RegionConfigManager
...
RESOURCE_MAP = {
    ...
    'inventory.Performance': 'PerformanceManager',
}
...
SERVICE_MAP = {
    ...
    'inventory.Performance': 'PerformanceService',
}
...
class CollectingManager(BaseManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.region_config_manager: RegionConfigManager = self.locator.get_manager('RegionConfigManager')
    ...
    def collecting_resources(self, plugin_info, secret_id, filters, domain_id, **kwargs):
        ...
        try:
            ...
            _LOGGER.debug("plugin_info['options'] >>>>>>> ")
            _LOGGER.debug(plugin_info['options'])
            _LOGGER.debug("collect_filter >>>>>>> ")
            _LOGGER.debug(collect_filter)
            # results = connector.collect(plugin_info['options'], secret_data.get('data', {}), collect_filter)

            # modify by psy 2024.4.9
            _LOGGER.debug("secret info >>>>>>> ")
            _LOGGER.debug(self.secret)
            _LOGGER.debug(self.secret['domain_id'])
            _LOGGER.debug(self.secret['project_id'])
            _LOGGER.debug(self.secret['provider'])
            _LOGGER.debug(self.secret['service_account_id'])

            # TODO (IF cloud_service 수집시만 동작하도록 수정할것.)
            # region_config_params ={
            #     'domain_id': self.secret['domain_id'],
            #     'project_id': self.secret['project_id'],
            #     'provider': self.secret['provider'],
            #     'service_account_id': self.secret['service_account_id']
            # }
            #
            # # get region_config_data from identity
            # _LOGGER.debug('[collect] get region_config_data from identity')
            # query = {}
            # _db_data = self.region_config_manager.list_available_region(
            #     region_config_params, query
            # )
            # _LOGGER.debug('[collect] region_config_data: >>>>> %s' % _db_data)

            # available_list = []
            # for _ in _db_data['results']:
            #     _LOGGER.debug(f" region_config : {_['region_config']}")
            #     for __ in _['region_config']:
            #         if __['is_available'] == 'TRUE':
            #             available_list.append(__['region_name'])

            # _LOGGER.debug(f" available_list: <<<<<--------- {available_list}")
            # available_list_params = {'available_region_config': available_list}

            # old -----------------
            # results = connector.collect(region_config_params, secret_data.get('data', {}), collect_filter)
            # results = connector.collect(available_list_params, secret_data.get('data', {}), collect_filter)
            results = connector.collect(plugin_info['options'], secret_data.get('data', {}), collect_filter)
    ...
    def _process_single_result(self, resource, params):
        ...
        print("========================= _process_single_result :", resource_type)
