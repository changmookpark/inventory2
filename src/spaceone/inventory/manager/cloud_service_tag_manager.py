...
class CloudServiceTagManager(BaseManager):
    ...
    def create_performance_tags_by_new_tags(self, cloud_service_vo, new_tags):
        for tag in new_tags:
            params = {
            ...
            self.create_performance_tag(params)

    def list_performance_tags(self, query=None):
        if query is None:
            query = {}
        return self.performance_tag_model.query(**query)

    def stat_performance_tags(self, query):
        return self.performance_tag_model.stat(**query)

    def filter_performance_tags(self, **conditions):
        return self.performance_tag_model.filter(**conditions)
