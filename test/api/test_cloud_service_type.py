...
class TestCloudServiceType(unittest.TestCase):
    config = utils.load_yaml_from_file(
        ...
        os.environ.get('SPACEONE_CONFIG_FILE', './config.yml'))
    ...
    def setUpClass(cls):
        ...
        cls.inventory_v1 = pygrpc.client(endpoint='localhost:50051/v1')
        cls.identity_v1 = pygrpc.client(endpoint='10.99.98.27:50051/v1')
        cls._create_domain()
    ...
    def test_create_cloud_service_type(self, name=None, provider=None, group=None):
        ...
        """
        print(f'test_create_cloud_service_type start ')
        if name is None:
            name = utils.random_string()
        ...
        print(f'self.inventory_v1.CloudServiceType.create call ')
        print(f'self.inventory_v1.CloudServiceType.create info {self.inventory_v1}')

        self.cloud_service_type = self.inventory_v1.CloudServiceType.create(
            params, metadata=(('token', self.owner_token),)
        )
        print(f'cloud_service_type finished')

        self._print_data(self.cloud_service_type, 'test_create_cloud_service_type')
        print(f'cloud_service_type print data')
        print(f'cloud_service_type {self.cloud_service_type}')
